package com.example.scannerai.data.ml.classification

import android.graphics.Bitmap
import android.graphics.Rect
import android.media.Image
import android.util.Log
import com.example.scannerai.data.ml.classification.utils.ImageUtils
...
class TextAnalyzer: ObjectDetector {
    ...
    override suspend fun analyze(
        ...
        withContext(Dispatchers.Default) {
            ...
            Log.d("TextAnalyzer", "originalBitmap: ${convertImageToBitmap.width}, ${convertImageToBitmap.height}")
            Log.d("TextAnalyzer", "croppedBitmap: ${croppedBitmap.width}, ${croppedBitmap.height}")
            croppedBit = croppedBitmap

            text = detector.process(InputImage.fromBitmap(croppedBitmap, 0)).await()

            //text = detector.process(InputImage.fromMediaImage(mediaImage, rotationDegrees)).await()
        }
            return if (text != null) {
